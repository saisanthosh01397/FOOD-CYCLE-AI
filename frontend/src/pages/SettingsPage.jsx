             ...
             {activeTab === 'account' && (
               <motion.div key="account" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                 ...
                 <div className="space-y-5 max-w-lg">
                   <div>
                     <label className="block text-sm font-semibold mb-1.5 text-[var(--text-secondary)]">Full Name</label>
                     <Input type="text" defaultValue={user?.full_name} icon={User} />
                   </div>
                   <div>
                     <label className="block text-sm font-semibold mb-1.5 text-[var(--text-secondary)]">Email Address</label>
                     <Input type="email" defaultValue={user?.email} disabled className="opacity-60 cursor-not-allowed" />
                     <p className="text-xs font-medium text-[var(--text-muted)] mt-1.5">Email cannot be changed directly. Contact an administrator.</p>
                   </div>
                   <div className="pt-4">
             ...
             {activeTab === 'appearance' && (
               <motion.div key="appearance" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                 <div className="border-b dark:border-slate-800 pb-4 mb-6">
                   ...
                   <p className="text-sm text-[var(--text-muted)] mt-1">Customize the interface.</p>
                 </div>
                 
                 <div className="max-w-xl">
                    <h4 className="font-semibold text-[var(--text-secondary)] mb-4">Theme Preference</h4>
                    <div className="grid grid-cols-2 gap-4">
                       <button onClick={() => darkMode && toggleTheme()} className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-4 transition-all duration-300 ${!darkMode ? 'border-brand-500 bg-brand-50/50 shadow-sm' : 'border-[var(--border)] hover:border-brand-300 dark:hover:border-slate-700'}`}>
                          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${!darkMode ? 'bg-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800'}`}>
                            <Monitor className={`w-8 h-8 ${!darkMode ? 'text-brand-500' : 'text-[var(--text-muted)]'}`}/>
                          </div>
                          <span className={`font-bold ${!darkMode ? 'text-slate-900' : 'text-[var(--text-muted)]'}`}>Light Mode</span>
                       </button>
                       <button onClick={() => !darkMode && toggleTheme()} className={`p-6 rounded-2xl border-2 flex flex-col items-center gap-4 transition-all duration-300 ${darkMode ? 'border-brand-500 bg-brand-900/10 shadow-sm' : 'border-[var(--border)] hover:border-brand-300 dark:hover:border-slate-700'}`}>
                          <div className={`w-16 h-16 rounded-full flex items-center justify-center ${darkMode ? 'bg-slate-900 shadow-sm' : 'bg-slate-100 dark:bg-slate-800'}`}>
                            <Monitor className={`w-8 h-8 ${darkMode ? 'text-brand-400' : 'text-[var(--text-muted)]'}`}/>
                          </div>
                          <span className={`font-bold ${darkMode ? 'text-white' : 'text-[var(--text-muted)]'}`}>Dark Mode</span>
                       </button>
                    </div>
                 </div>
               </motion.div>
             ...
             {activeTab === 'notifications' && (
               <motion.div key="notifications" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                 <div className="border-b dark:border-slate-800 pb-4 mb-6">
                   ...
                   <p className="text-sm text-[var(--text-muted)] mt-1">Control how and when we alert you.</p>
                 </div>
                 <div className="space-y-4 max-w-2xl">
                    {[
                      ...
                      <div key={i} className="flex items-center justify-between p-5 rounded-2xl border dark:border-slate-800 bg-[var(--surface-hover)] hover:border-brand-200 dark:hover:border-slate-700 transition-colors">
                         <div>
                            <div className="font-bold text-[var(--text-primary)]">{item.title}</div>
                            <div className="text-sm text-[var(--text-muted)] font-medium mt-1">{item.desc}</div>
                         </div>
             ...
             {activeTab === 'system' && user?.role === 'Administrator' && (
               <motion.div key="system" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                 <div className="border-b dark:border-slate-800 pb-4 mb-6">
                   ...
                   <p className="text-sm text-[var(--text-muted)] mt-1">Manage infrastructure backups and restoration.</p>
                 </div>
                 ...
                 <div className="grid md:grid-cols-2 gap-6">
                   <Card className="p-6 md:p-8 flex flex-col items-start bg-slate-50/50 dark:bg-slate-900/30 hover:border-blue-300 dark:hover:border-blue-800 transition-colors">
                     ...
                     <h4 className="font-bold text-lg mb-2 tracking-tight text-[var(--text-primary)]">Backup Database</h4>
                     <p className="text-sm text-[var(--text-muted)] mb-8 leading-relaxed flex-1">Create a secure snapshot of all users, logs, and AI predictions to prevent data loss.</p>
                   ...
                   <Card className="p-6 md:p-8 flex flex-col items-start bg-slate-50/50 dark:bg-slate-900/30 hover:border-amber-300 dark:hover:border-amber-800 transition-colors">
                     ...
                     <h4 className="font-bold text-lg mb-2 tracking-tight text-[var(--text-primary)]">Restore Database</h4>
                     <p className="text-sm text-[var(--text-muted)] mb-8 leading-relaxed flex-1">Revert the database to the most recent backup snapshot. Warning: Destructive action.</p>
             ...
             {activeTab === 'security' && user?.role === 'Administrator' && (
               <motion.div key="security" variants={tabVariants} initial="hidden" animate="visible" exit="exit" className="space-y-6">
                 <div className="border-b dark:border-slate-800 pb-4 mb-6">
                   ...
                   <p className="text-sm text-[var(--text-muted)] mt-1">Monitor authentication events and access control.</p>
                 </div>
                 <div className="flex flex-col items-center justify-center p-12 text-[var(--text-muted)] border border-dashed dark:border-slate-800 rounded-2xl">
